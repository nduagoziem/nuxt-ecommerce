...
const { data: response } = await useFetch(`${config.public.apiBase}/phones?fields[]=name&fields[]=price&fields[]=brand`);
          ...
          <div class="w-full flex flex-col" v-for="data in response.data" :key="data.hashid">
            <a :href="`/categories/phones/${data.brand}/${data.hashid}`">
              <div
                ...
                <img :src="data.media[0]" :alt="data.name" class="object-cover w-full h-full" />
              </div>
